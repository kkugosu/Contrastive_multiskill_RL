...
class VIC(BASE.BaseControl):
    ...
    def reward(self, *trajectory):
        ...
        initial_skill_maybe = torch.zeros(size=(1000, self.sk_n)).to(self.device) + 1/self.sk_n
        tmp_state = t_p_s[0].unsqueeze(0)
        while i < len(n_d):
            if n_d[i] == 1:
                tmp_state = t_p_s[i].unsqueeze(0)
                initial_pair = torch.cat((tmp_state, tmp_state), -1)
                initial_skill_maybe[i] = self.discriminator(initial_pair)
            current_pair = torch.cat((tmp_state, t_p_s[i].unsqueeze(0)), -1)
            skill_maybe[i] = self.discriminator(current_pair)
            i = i + 1
        entropy = torch.sum(-initial_skill_maybe*torch.log(initial_skill_maybe), -1)
    ...
    def update(self, memory_iter, *trajectory):
        ...
        while i < memory_iter:
            ...
            self.optimizer.step()
        loss_ary = torch.cat((loss2_ary, torch.sum(loss1).unsqueeze(0)), -1)
        return loss_ary
